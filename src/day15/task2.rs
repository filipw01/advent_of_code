    ...
    let cave: HashMap<Coord, usize> = input
        ...
        .flat_map(|(y, line)| {
            line.chars()
                ...
                .flat_map(|(x, c)| {
                    let mut tiles = vec![];
                    for x_multiplier in 0..5 {
                        for y_multiplier in 0..5 {
                            tiles.push((
                                (
                                    (x + x_multiplier * input_size),
                                    (y + y_multiplier * input_size),
                                ),
                                ((c.to_digit(10).unwrap() as usize) + x_multiplier + y_multiplier
                                    - 1)
                                    % 9
                                    + 1,
                            ));
